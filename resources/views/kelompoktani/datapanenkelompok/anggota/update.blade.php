...
@section('content')
    <div class="container-fluid">
        ...
        <div class="card w-100" style="background-color: #D9D9D9">
            <div class="card-body">
                <form
                    action="/data-panen-kelompok/{{ $panenanggota->id_tanggal_panen }}/{{ $panenanggota->id_anggota_tervalidasi }}/updateData"
                    method="POST">
                    @method('put')
                    @csrf
                    <div class="mb-3">
                        ...
                        <select class="custom-select" name="id_anggota_tervalidasi">
                            ...
                            @foreach ($nama_anggota as $item)
                                <option value="{{ $item->id_anggota_tervalidasi }}"
                                    {{ $panenanggota->id_anggota_tervalidasi == $item->id_anggota_tervalidasi ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="tonase_panen" id="tonaseInput"
                            value="{{ $panenanggota->tonase_anggota }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // const tonasepanenInput = document.querySelector('input[name="tonase_panen"]');
            ...
            form.addEventListener('submit', function(event) {
                ...
                fetch('/check-data/data-panen-update/{{ $id_tanggal_panen }}', {
                        method: 'POST',
                        headers: {
                        ...
                        body: JSON.stringify({
                            // tonase_panen: tonasepanenInput.value,
                            id_anggota_tervalidasi: idAnggotaSelect.value,
                        }),
                    })
                    ...
                    .then(data => {
                        if (data.isUnique) {
                            ...
                            showAlert('Data panen sudah tersedia.');
                        }
                    })
